define(function(require) {
  ...
  module.controller('ConfigurationsListCtrl', ConfigurationsListCtrl);
  ConfigurationsListCtrl.$inject = ['$state', 'PaginationFactory', 'ConfigurationsSearchResource', 'ConfigurationsFactory'];

  function ConfigurationsListCtrl($state, pagination, ResourceSearch, dataService) {
    var vm = this;
    init();
    ...
    vm.path = function(id) {
      if (id > 0) {
        $state.go('home.configurations.edit', {id: id});
      } else {
        $state.go('home.configurations.new');        
      }
    }
    ...
    function init() {
      var ctrlName = 'ConfigurationsListCtrl';
