define(function(require) {
  ...
  module.controller('ResolutionsListCtrl', ResolutionsListCtrl);
  ResolutionsListCtrl.$inject = ['$state', 'PaginationFactory', 'ResolutionsSearchResource', 'ResolutionsFactory'];

  function ResolutionsListCtrl($state, pagination, ResourceSearch, dataService) {
    var vm = this;
    init();
    ...
    vm.path = function(id) {
      if (id > 0) {
        $state.go('home.resolutions.edit', {id: id});
      } else {
        $state.go('home.resolutions.new');        
      }
    }
    ...
    function init() {
      var ctrlName = 'ResolutionsListCtrl';
